function loadWorldmap(){
  var foodName = window.location.href.match(/food=([a-zA-Z_]+)$/)[1];
    ...
    d3.json("result.json", function (error, resJson) {
      ...
      for(var lang in resJson[food]){
        ...
        if(!lfObj.hasOwnProperty(lang)){
          lfObj[lang] = {};
          lfObj[lang]["foodName"] = food;

        }
        lfObj[lang]["doc_count"] = count;
      }
      //
  ...
  function processWorldD(world, data) {
    ...
    for(var country in data) {
      ...
      for(var wdx=0; wdx < world.objects.subunits.geometries.length; wdx++) {
        var cName = world.objects.subunits.geometries[wdx].id.toUpperCase();
        if (cCode === cName) {
          for(var entry in data[country]){
            world.objects.subunits.geometries[wdx].properties[entry] = data[country][entry];
          }
        }
      }
    }
    ...
    countries.append('svg:text')
        ...
        .attr('x', 0)
        .attr('dy', 20)
        .text(function(d) { return d.properties["foodName"]? d.properties["foodName"]: ''; })
  }
